#phylogeobears v 0.21 (installed 7/7/16)

rm(list = ls())
options(stringsAsFactors = FALSE)

# Add filenames for the newick tree file and the geographic data file
# See below for file descriptions and cautions
treefile <- "pruned_tree2.newick"
geodata <- "Pmac_geog_data_pruned2.txt"

#set path to working directory (input and output files)
workdir <- "~/Documents/Mol Ecol Lab/ Projects/ Sperm whales/Pmac_mitogenomics/Phylogeog_BioGeoBears_results/Pruned_tree_12pops"
# set description, for naming output files
description <- "Pmac_12pops"

# from http://phylo.wikidot.com/biogeobears
#######################################################
...
# (from Ree & Smith 2008)
# "trfn" = "tree file name"
#trfn = np(paste(addslash(extdata_dir), "Psychotria_5.2.newick", sep=""))  #test file
trfn = np(paste(addslash(wd), treefile, sep=""))
...
tr
plot(tr)
#title("Example Psychotria phylogeny from Ree & Smith (2008)")  #test data title
title(description)
...
# (from Ree & Smith 2008)
#geogfn = np(paste(addslash(extdata_dir), "Psychotria_geog.data", sep="")) #test geog file
geogfn = np(paste(addslash(wd), geodata, sep=""))
#need to create phylip-formatted geography file (see above)
...
######  For negative branches, searched through newick tree to find "-" and changed 
#       negative number to 0.00001

# For a slow analysis, run once, then set runslow=FALSE to just 
# load the saved result.
runslow = TRUE
#resfn = "Psychotria_DEC_M0_unconstrained_v1.Rdata"
resfn = paste(description,"_DEC",".Rdata", sep="")
if (runslow)
{
...
resfn = paste(description, "_DEC+J.Rdata", sep="")
runslow = TRUE
if (runslow)
{
...
#For custom colors, need to create the color matrix, import it, 
#and source custom script for custom colors.

source("plot_BioGeoBEARS_results.R")
#PM created new function for plot_BioGeoBears_results by doing:
# dump("plot_BioGeoBEARS_results", file = "plot_BioGeoBEARS_results.R")
# then opening the new R file as a script and replacing the line:
#   color_matrix = get_colors_for_numareas(length(areanames))
# with
#   color_matrix = color_matrix_4areas

color_matrix_4areas <- read.csv("color_matrix_4areas.csv", row.names = 1)
# color matrix .csv for 4 areas looks like:
#       V1	V2	V3	V4
#red	  0	  255	0	  205
#green	0	  255	205	0
#blue	  205	0	  0	  0
...
pdffn = paste(description,"_DEC+J_MO_unconstrained_v1.pdf", sep="")
pdf(pdffn, width=6, height=6)
# PM adjusted width and height for 81 hap tree, longer tip names
#######################################################
# Plot ancestral states - DEC
#######################################################
#analysis_titletxt ="BioGeoBEARS DEC on Psychotria M0_unconstrained"
analysis_titletxt = paste(description,"DEC MO unconstrained", sep = " ")

# Setup
results_object = resDEC
scriptdir = np(system.file("extdata/a_scripts", package="BioGeoBEARS"))

# States (cex is scaling factor relative to the default)
# plotlegend = true, make a (separate) plot with a legend giving the colors
# for each state/range, using colors_legend (assigns colors based on number of states?)
...
analysis_titletxt = paste(description, " DEC+J MO_unconstrained", sep = "")
...
resfn = paste(description,"_DivaLike_MO_unconstrained_v1.Rdata", sep="")
if (runslow)
{
  res = bears_optim_run(BioGeoBEARS_run_object)
  res    
...
resfn = paste(description,"_Diva+J_MO_unconstrained_v1.Rdata", sep="")
runslow = TRUE
if (runslow)
{
  #sourceall("/Dropbox/_njm/__packages/BioGeoBEARS_setup/")
...
pdffn = paste(description,"_Diva_vs_Diva+J_MO_unconstrained_v1.pdf", sep="")
pdf(pdffn, width=6, height=6)
...
analysis_titletxt = paste(description," DIVALIKE M0_unconstrained")
...
analysis_titletxt =paste(description, " DIVALIKE+J M0_unconstrained")
...
resfn = paste(description, "_Bayarea_MO_unconstrained_V1.Rdata", sep="")
if (runslow)
{
  res = bears_optim_run(BioGeoBEARS_run_object)
  res    
...
resfn = paste(description, "_Bayarea+J_MO_unconstrained_v1.Rdata", sep="")
runslow = TRUE
if (runslow)
{
  res = bears_optim_run(BioGeoBEARS_run_object)
...
pdffn = paste(description, "_Bayarea_vs_Bayarea+J_MO_unconstrained_v1.pdf", sep="")
pdf(pdffn, width=6, height=6)
...
analysis_titletxt = paste(description, "bayarealike MO unconstrained", sep=" ")
...
analysis_titletxt = paste(description, "bayarealike+J MO unconstrained", sep=" ")
...
restable = NULL   #restable, the modified table, or wt_vBest, the Akaike Weights results??
teststable = NULL
...
#
# AIC and AICc model weights are also shown, giving a sense of the relative probability of the 
# two models.
#
...
# > teststable #results of likelihood ratio test for each pair of models. Lower AIC is better.
# 
